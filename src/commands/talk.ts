...
export const talkCommand = {
  data: new SlashCommandBuilder()
    .setName('talk')
    .setDescription('Talk to the pet')
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      // Check cooldown
      const lastInteraction = await getLastInteractionTime(serverId, userId, 'talk');
      if (lastInteraction) {
        ...
        if (timeSinceLastInteraction < COOLDOWN_MS) {
          ...
          await interaction.editReply({
            content: `Please wait ${remainingSec}s before talking to the pet again.`,
          });
          return;
        }
      }
      ...
      const newHappiness = Math.min(petState.happiness + 10, 100);
      ...
      await updatePetState(serverId, {
        happiness: newHappiness,
        mood: newMood,
        lastTalkedAt: new Date(),
        lastInteractionAt: new Date(),
      });

      // Log interaction
      await logInteraction(serverId, userId, 'talk');

      // Get response message
      const responseMessage = getRandomMessage('talk');
      ...
      await interaction.editReply({
        content: 'An error occurred while talking to the pet.',
      });
    }
  },
};
